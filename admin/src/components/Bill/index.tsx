...
const Bill = forwardRef<HTMLDivElement, BillProps>(
  (
    ...
    return createPortal(
      <>
        <div
          className="serve-overlay fixed inset-0 z-[100]"
          onClick={onClose}
        />
        <div
          role="dialog"
          aria-modal="true"
          className={`serve-modal fixed left-1/2 top-1/2 z-[101] mx-auto max-h-[90vh] w-[95vw] max-w-[800px] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-lg p-6 ${className ?? ""}`}
        >
          {/* Action Buttons */}
          ...
          {/* Header */}
          <div ref={ref} id="bill-print" className="print-surface bg-white text-black">
            ...
            <div className="mt-4 space-y-1 text-sm">
              ...
              <div className="flex justify-between font-bold text-lg border-t-2 border-black pt-2 mt-2">
                <span>Total:</span>
                <span>
                  Rs. {Number(orderData?.totalAmount ?? 0).toFixed(2)}
                </span>
              </div>
            </div>
