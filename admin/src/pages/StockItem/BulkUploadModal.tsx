...
import {
  ...
  type ImportStockItemsResponse,
} from "@/redux/services/stockItem";
...
const TEMPLATE_SAMPLE_ROWS = [
  ["Milk", "ltr", "Dairy", "50", "4", "50", "", "1"],
  ["Chicken", "kg", "Meat", "450", "10", "450", "", "2"],
  ["Oil", "ltr", "Grocery", "280", "5", "280", "", ""],
];

type ImportSummary = NonNullable<ImportStockItemsResponse["data"]>;
...
export default function BulkUploadModal({
  ...
  const [importStockItems, { isLoading }] = useImportStockItemsMutation();
  ...
  const runImport = useCallback(async () => {
    ...
    try {
      const response = await importStockItems({ file, dryRun: false }).unwrap();
      if (!response?.data) {
      ...
      setPreview(null);
      Toast(response.msg || "Stock items imported.", "success");
      onImported();
    } catch (error) {
  ...
  }, [file, importStockItems, onImported]);

  const downloadTemplate = useCallback(async () => {
    try {
      await exportToExcel({
        title: "Serve Stock Import Template",
      ...
      <div className="max-h-[70vh] space-y-5 overflow-y-auto p-6">
        <div className="flex flex-col gap-3 rounded-xl border border-[var(--serve-border)] bg-[var(--serve-surface-2)] p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-[13px] text-[var(--serve-muted)]">
              Name and Measuring Unit are required. Units and suppliers must
              already exist; groups are created automatically.
            </p>
          </div>
          <button
            type="button"
        ...
        {result ? (
          <p className="flex items-start gap-2 rounded-lg border border-[color-mix(in_srgb,var(--serve-positive)_30%,var(--serve-border))] bg-[color-mix(in_srgb,var(--serve-positive)_10%,transparent)] px-3 py-2 text-[13px] text-[var(--serve-fg)]">
            ...
            />
            {result.created} stock item(s) added to inventory.
          </p>
        ) : null}
        ...
        <div className="flex flex-col gap-2 border-t border-[var(--serve-border)] pt-4 sm:flex-row sm:justify-end">
          ...
          {!result ? (
            ...
            >
              ...
              {importable ? `Import ${preview?.created} item(s)` : "Import"}
            </button>
          ) : (
            <button
              type="button"
