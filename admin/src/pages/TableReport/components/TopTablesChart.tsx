...
const TopTablesChart: React.FC<TopTablesChartProps> = ({ data, isLoading }) => {
  ...
  const chartData = data.slice(0, 5).map((table: any) => ({
    name: `Table ${table.tableNo || table.tableNo}`,
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
      <h2 className="text-lg font-semibold mb-4">Top 5 Performing Tables (Today)</h2>
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
            ...
            <XAxis 
              type="number" 
              tickFormatter={(value) => `${CurrencySign}${value.toLocaleString()}`}
              style={{ fontSize: "12px" }}
            />
            <YAxis 
              type="category" 
            ...
            />
            <Tooltip 
              formatter={(value: number) => [`${CurrencySign}${value.toLocaleString()}`, "Revenue"]}
