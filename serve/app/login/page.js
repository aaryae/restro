...
function LoginForm() {
  ...
  async function finish(data) {
    ...
    if (password) sessionStorage.setItem('serve_pending_password', password)
  ...
  return (
    <main className="min-h-screen grid lg:grid-cols-2 bg-milk text-ink">
      ...
      {/* Left — original brand panel (left-aligned, not centered) */}
      <aside className="relative hidden lg:flex flex-col justify-between overflow-hidden bg-espresso text-cream px-12 xl:px-16 py-12 pl-[8.125rem]">
        ...
        <Link
          ...
          <Image
            ...
            width={916}
            height={444}
            className="logo-invert block h-16 xl:h-20 w-auto max-w-[220px]"
            priority
          />
        </Link>

        <div className="relative z-10 max-w-2xl -translate-y-14 xl:-translate-y-16">
          <p className="font-dmono text-[0.7rem] tracking-[0.2em] uppercase text-accent mb-5">
            Cafe management · Nepal
          </p>
          <h1 className="font-syne font-extrabold tracking-[-0.04em] leading-[0.98] text-[clamp(3.4rem,5.8vw,4.85rem)]">
            Less
            <span className="block text-cream/40 whitespace-nowrap">
              chaos. More cups.
            </span>
            <span className="block">More control.</span>
            <span className="block">Serve.</span>
          </h1>
          <p className="mt-6 text-cream/65 text-base leading-relaxed max-w-sm font-light">
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center items-start pl-10 pr-6 sm:pl-16 sm:pr-10 xl:pl-24 xl:pr-14 pb-12 pt-2">
          <div className="max-w-[460px] w-full text-left">
            ...
            <div
              ...
              <div className="mb-5">
                <p className="text-sm font-medium text-espresso">Verify your email</p>
              </div>
            )}
            ...
            <form
              ...
              <div
                className={`grid transition-[grid-template-rows] duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                  mode === 'login' ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'
                }`}>
                <div className="overflow-hidden min-h-0">
                  <div
                    className={`pb-2 transition-opacity duration-300 ${
            ...
            <p className="mt-5 text-sm text-muted">
              ...
              <button
                type="button"
                onClick={() => switchMode(copy.otherMode)}
