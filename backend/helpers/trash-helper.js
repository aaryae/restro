  ...
  return trashItemModel.create(
    {
      resourceType,
      resourceId: data.id,
      displayName: String(displayNameFromRecord(resourceType, data)),
      payload: data,
      deletedBy,
      deletedByName,
...
const restoreTrashItem = async (req) => {
  ...
  const Model = RESOURCE_MODELS[item.resourceType];
  if (!Model) {
    return {
      status: 400,
      success: false,
  ...
  try {
    ...
    try {
      const { id: _omit, createdAt, updatedAt, ...rest } = payload;
      const created = await Model.create(rest);
      await item.destroy();
      return {
        status: 200,
        success: true,
      ...
      return {
        status: 500,
        success: false,
        message: retryError.message || "Failed to restore item",
        data: null,
      };
    }
  }
